...
# Preserve OPAL-owned source/config while Flutter creates only the native scaffold.
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
...
flutter create \
  --platforms=android \
  --org com.opalschool2016 \
  --project-name opal_learning_app \
  --no-pub \
  .
...
echo "Android scaffold is ready: $ROOT/android"
