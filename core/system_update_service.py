...
def get_current_version() -> CurrentVersion:
    ...
    name = str(marker.get("version_name") or "")
...
def save_uploaded_update(uploaded_file, *, username: str) -> LocalVersionRecord:
    ...
    try:
        ...
        if exc.errno == errno.ENOSPC:
            raise ValidationError("المساحة المتاحة لا تكفي لحفظ ملف التحديث.") from exc
        raise ValidationError("تعذر حفظ ملف التحديث على الخادم.") from exc
    finally:
        temporary_path.unlink(missing_ok=True)
...
def _patch_urls(path: Path) -> None:
    ...
    import_line = "from . import system_update_views"
    if import_line not in content:
        ...
        content = "\n".join(lines) + "\n"
    route_line = '    path("updates/", system_update_views.system_updates, name="system_updates"),'
    if 'name="system_updates"' not in content and "name='system_updates'" not in content:
        close_index = content.rfind("]")
        if close_index == -1:
            raise SystemUpdateError("تعذر تحديد urlpatterns داخل core/urls.py.")
        content = content[:close_index] + route_line + "\n" + content[close_index:]
...
def _reapply_update_tool_overlay() -> None:
    ...
    root = project_root()
    for relative in (
        Path("core/system_update_service.py"),
        Path("core/system_update_views.py"),
        Path("core/tests_system_updates.py"),
        Path("templates/core/system_updates.html"),
    ):
...
def _replace_project_code(source_root: Path) -> None:
    ...
    for child in source_root.iterdir():
        if child.name in PROTECTED_TOP_LEVEL or child.name == MANIFEST_NAME:
            continue
...
def _run_post_restore_checks(*, run_migrations: bool) -> None:
    python_bin = '/home/Opalschool2016/opal_school/venv/bin/python'
...
def _write_deployed_marker(*, version_name: str, source: str, username: str, safety_snapshot: str) -> None:
    status = get_git_status()
    data = {
        "version_name": version_name,
        "source": source,
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="opal_restore_", dir=private_storage_root()) as temporary_directory:
        ...
        try:
            ...
            # إعادة كود نسخة الأمان تلقائيًا. ترحيلات قاعدة البيانات لا يتم عكسها تلقائيًا.
            try:
            ...
            raise SystemUpdateError(
                f"فشلت الاستعادة وتمت إعادة كود النسخة السابقة تلقائيًا. السبب: {original_exc}"
            ) from original_exc
    ...
    _append_audit("version_restored", username, f"version={final_name} source={source_label} safety={safety.filename}")
...
def push_current_system_to_github(*, username: str) -> GitPushResult:
    ...
    with lock_path.open("w", encoding="utf-8") as lock_file:
        ...
        _run(['/home/Opalschool2016/opal_school/venv/bin/python', "manage.py", "check"], timeout=180)
        _run(['/home/Opalschool2016/opal_school/venv/bin/python', "manage.py", "makemigrations", "--check", "--dry-run"], timeout=180)
