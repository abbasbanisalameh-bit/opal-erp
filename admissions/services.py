...
def grade_fee_amount(grade, school=None, academic_year=None):
    ...
    fee = None
    if academic_year:
        fee = GradeFee.objects.filter(school=school, grade=grade, academic_year=academic_year, is_active=True).first()
    if fee is None:
        fee = GradeFee.objects.filter(school=school, grade=grade, academic_year__isnull=True, is_active=True).first()
    if fee is None:
        fee = GradeFee.objects.filter(school=school, grade=grade, is_active=True).first()
...
def create_student_registration(form, user=None):
    ...
    student = Student.objects.create(
        ...
        fees_total=totals["net_total"],
        fees_paid=totals["first_payment"],
