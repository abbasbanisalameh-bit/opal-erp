...
AUDIT = Path("OPAL_CSS_AUTHORITY_AUDIT_R89.json")
OUT_JSON = Path("OPAL_CSS_DUPLICATE_ANALYSIS_R89.json")
OUT_MD = Path("OPAL_CSS_DUPLICATE_ANALYSIS_R89.md")

if not AUDIT.exists():
    raise SystemExit(f"ERROR: Audit file not found: {AUDIT}")

data = json.loads(AUDIT.read_text(encoding="utf-8"))

duplicates = data.get("exact_duplicate_blocks", [])
...
report = {
    ...
    "exact_duplicate_blocks": len(duplicates),
    ...
    "safety_rule": "REPORT ONLY. No CSS files were modified or deleted.",
}
...
lines = [
    ...
    "- REVIEW_CROSS_FILE: appears across multiple CSS files",
