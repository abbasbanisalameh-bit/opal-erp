from django.urls import path

from . import views

app_name = "exams"

urlpatterns = [
    path("", views.gradebook, name="exam_list"),
    path("gradebook/", views.legacy_gradebook, name="gradebook"),
    path("definitions/", views.exam_definitions, name="exam_definitions"),
    path("dashboard/", views.exam_dashboard, name="exam_dashboard"),
    path("cycles/", views.exam_cycle_center, name="exam_cycle_center"),
    path("scope-options/", views.exam_scope_options, name="exam_scope_options"),
    path("add/", views.exam_create, name="exam_create"),
    path("marks/", views.mark_list, name="mark_list"),
    path("marks/add/", views.mark_create, name="mark_create"),
    path("students/<int:student_id>/record/", views.student_record, name="student_record"),
    path("students/<int:student_id>/record/print/", views.student_record_print, name="student_record_print"),
    path("students/<int:student_id>/report-card/", views.student_report_card, name="student_report_card"),
    path("students/<int:student_id>/report-card/issue/", views.issue_student_report_card, name="issue_student_report_card"),
    path("<int:exam_id>/marks/bulk/", views.exam_marks_bulk, name="exam_marks_bulk"),
    path("<int:exam_id>/export/csv/", views.exam_export_csv, name="exam_export_csv"),
    path("<int:exam_id>/action/", views.exam_action, name="exam_action"),
    path("<int:exam_id>/", views.exam_detail, name="exam_detail"),
    path("<int:exam_id>/edit/", views.exam_update, name="exam_update"),
    path("<int:exam_id>/delete/", views.exam_delete, name="exam_delete"),
]
